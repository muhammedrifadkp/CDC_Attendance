...
import { TIME_SLOTS, getCurrentTimeSlot, getTimeSlotLabel } from '../../utils/timeSlots'
...
const LabOverviewFixed = () => {
  ...
  // Auto-select time slot on component mount using centralized utility
  useEffect(() => {
    if (!selectedTimeSlot) {
      const currentSlot = getCurrentTimeSlot()
      if (currentSlot) {
        // Convert from centralized format to local format
        const localTimeSlot = currentSlot.id.replace(' AM', '').replace(' PM', '').replace(' - ', '-')
        setSelectedTimeSlot(localTimeSlot)
        setAutoSelected(true)
        toast.success(`🕒 Auto-selected current time slot: ${currentSlot.label}`)
      }
    }
  }, [])
  ...
  const getPCStatusColor = (pc, booking) => {
    ...
    if (booking) {
      ...
      switch (attendanceStatus) {
        case 'present':
          return 'bg-orange-500 border-orange-600 text-white' // 🟠 Orange: Booked with student present
        case 'absent':
          return 'bg-red-500 border-red-600 text-white' // 🔴 Red: Booked but student absent
        case 'late':
