...
export const pcAPI = {
  ...
  // Create sample PCs
  createSamplePCs: async () => {
    try {
      console.log('📡 API Request: POST /lab/pcs/create-sample')
      const response = await api.post('/lab/pcs/create-sample')
      console.log('✅ API Response: POST /lab/pcs/create-sample', response.data)
      return response.data
    } catch (error) {
      console.error('❌ API Error: POST /lab/pcs/create-sample', error)
      throw error
    }
  }
}
