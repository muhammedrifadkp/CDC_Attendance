...
// @desc    Create sample PC data
// @route   POST /api/lab/pcs/create-sample
// @access  Private (Admin/Teacher)
const createSamplePCs = asyncHandler(async (req, res) => {
  // Check if PCs already exist
  const existingPCs = await PC.countDocuments();
  if (existingPCs > 0) {
    return res.json({
      message: 'Sample PCs already exist',
      count: existingPCs
    });
  }

  // Create sample PCs for 4 rows with 10 PCs each
  const samplePCs = [];
  for (let row = 1; row <= 4; row++) {
    for (let position = 1; position <= 10; position++) {
      const pcNumber = `PC${row}${position.toString().padStart(2, '0')}`;
      samplePCs.push({
        pcNumber,
        row: row.toString(),
        position,
        ...
        notes: 'Sample PC for testing'
      });
    }
  }

  const createdPCs = await PC.insertMany(samplePCs);

  res.status(201).json({
    message: 'Sample PCs created successfully',
    count: createdPCs.length,
    pcs: createdPCs
  });
});
